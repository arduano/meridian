...
pub(super) fn wire_modify_pass_selection(app: &App) {
...
pub(super) fn wire_modify_control_updates(app: &App) {
    let app_weak = app.as_weak();
...
pub(super) fn wire_modify_config_editor(app: &App) {
    let app_weak = app.as_weak();
...
pub(super) fn wire_modify_config_reset(app: &App) {
    let app_weak = app.as_weak();
...
pub(super) fn wire_modify_output_browse(app: &App) {
    let app_weak = app.as_weak();
...
pub(super) fn wire_modify_job_actions(
    app: &App,
    bridge: &UiCoreBridge,
    ...
    let start_shared_state = Arc::clone(shared_state);
    app.on_start_modify_job(move || {
        ...
        if let Err(message) = validate_modify_job_output_path(Path::new(selected.as_str()), &output)
        {
            set_modify_ui_failure(&app, &message);
            app.window().request_redraw();
            return;
        ...
        let events = match start_bridge.start_process_midi_file(
        ...
        if let Some(message) = events_error_message(&events) {
            ...
            apply_events_to_app(&app, &start_shared_state, &events);
        }
        app.window().request_redraw();
    });
    ...
    let cancel_shared_state = Arc::clone(shared_state);
    app.on_cancel_modify_job(move || {
        let Some(app) = app_weak.upgrade() else {
            return;
        };
        let events = match cancel_bridge.cancel_midi_file_process(&cancel_shared_state) {
            Ok(events) => events,
            Err(error) => {
                set_modify_ui_failure(&app, &error.to_string());
                app.window().request_redraw();
        ...
        if let Some(message) = events_error_message(&events) {
            ...
            apply_events_to_app(&app, &cancel_shared_state, &events);
        }
        app.window().request_redraw();
    });
}
