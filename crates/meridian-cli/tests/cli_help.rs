...
fn top_level_help_exposes_stdio_and_render_commands() {
    ...
    assert!(!stdout.contains("debug-piano-trail-classic-geometry"));
}

#[test]
fn legacy_stdio_alias_is_rejected() {
    let output = Command::new(cli_path())
        .args(["serve-json", "--help"])
        .output()
        .expect("run cli serve-json --help");

    assert!(!output.status.success(), "serve-json should be rejected");
    let stderr = String::from_utf8(output.stderr).expect("utf8 stderr");
    assert!(stderr.contains("unrecognized subcommand") || stderr.contains("found argument"));
}
